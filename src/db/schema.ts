...
/** 存储用户绑定的 Passkey 凭证 */
export const authenticators = novelcraft.table(
  "authenticators",
  {
    userId: uuid("user_id")
      .notNull()
  ...
  (table) => ({
    ...
    credentialIdIdx: index("authenticators_credential_id_idx").on(table.credentialID),
  }),
);

/** 存储临时 challenge */
export const passkeyChallenges = novelcraft.table(
  "passkey_challenges",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
/** 存储一次性登录令�?*/
export const passkeyLoginTokens = novelcraft.table(
  "passkey_login_tokens",
  {
  ...
  (table) => [
    index("passkey_login_tokens_expires_idx").on(table.expiresAt),
  ],
);
